...
class DevisResponse extends StatefulWidget {
  final Message message;
  final UserProvider userProvider;
  DevisResponse(this.message, this.userProvider);
  @override
  _DevisResponseState createState() => _DevisResponseState();
}

class _DevisResponseState extends State<DevisResponse> {
  String getMessagetitle(bool isMe) {
    return "${getTranslate(context, "DEVIS_FOR")} ${widget.message.offer.title}";
  }
  ...
  String getMessageSubtitle(bool isMe) {
    if (!isMe) {
      if (widget.message.response == true)
        ...
        return "${getTranslate(context, "NEGOCIATION")} :";
      else if (widget.message.response == false)
        return getTranslate(context, "DEVIS_REFUSED");
      else
        return getTranslate(context, "NEW_DEVIS_SENT");
    } else {
      if (widget.message.response == true)
        return getTranslate(context, "DEVIS_ACCEPTED");
      else if (widget.message.response == false && widget.message.text != '')
        ...
        return getTranslate(context, "NEW_DEVIS_RECEIVED");
    }
  }
                    ...
                    children: [
                              ...
                              child: ElevatedButton(
                                onPressed: () {
                                  Navigator.of(context).pushNamed(
                                      ...
                                      arguments: DevisDetailsArguments(null,
                                          widget.message.devis, false, false));
                                },
                                style: ButtonStyle(
                                  backgroundColor:
                                ...
                                child: Text(
                                    getTranslate(context, "CONSULT_DEVIS")),
                              ),
                            )
                          : SizedBox.shrink(),
                      ...
                      !isMe &&
                          ...
                          ? SizedBox(
                              height: 30,
                              child: ElevatedButton(
                                onPressed: !widget.message.offer.isAvailable
                                    ? null
                                    : () {
                                        Navigator.of(context).pushNamed(
                                ...
                                child:
                                    Text(getTranslate(context, "APPLY_AGAIN")),
                              ),
                            )
                          : SizedBox.shrink()
                    ],
