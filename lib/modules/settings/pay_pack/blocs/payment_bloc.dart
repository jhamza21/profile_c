    ...
    try {
      ...
      if (paymentIntentResults['error'] != null) {
        ...
        add(PaymentConfirmIntent(
            clientSecret: clientSecret, packId: event.packId));
      } else {
        emit(state.copyWith(status: PaymentStatus.failure));
      }
    } catch (e) {
  ...
  _onPaymentConfirmIntent(
    ...
    try {
      ...
      if (paymentIntent.status == PaymentIntentsStatus.RequiresConfirmation) {
        Map<String, dynamic> results = await _callEndpointIntentId(
            paymentIntentId: paymentIntent.id, packId: event.packId);
...
Future<Map<String, dynamic>> _callEndpointMethodId(
    {bool usestripeSdk, String paymentMethodId, int packId}) async {
...
Future<Map<String, dynamic>> _callEndpointIntentId(
    {String paymentIntentId, int packId}) async {
  String token = await SecureStorageService.readToken();
  ...
  final res = await http.post(Uri.parse(url),
      ...
      body:
          json.encode({'paymentIntentId': paymentIntentId, 'packId': packId}));
  return json.decode(res.body);
}
