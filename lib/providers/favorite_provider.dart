import 'dart:convert';

import 'package:flutter/material.dart';
import 'package:profilecenter/models/user.dart';
import 'package:profilecenter/utils/helpers/session_expired.dart';
import 'package:profilecenter/core/services/user_service.dart';

class FavoriteProvider extends ChangeNotifier {
  List<User> _favorites = [];
  bool _isFetched;
  bool _isError;
  bool _isLoading;

  List<User> get favorites => _favorites ?? [];
  bool get isFetched => _isFetched ?? false;
  bool get isError => _isError ?? false;
  bool get isLoading => _isLoading ?? false;

  void initialize() {
    _favorites = [];
    _isFetched = false;
    _isError = false;
    _isLoading = false;
  }

  void fetchFavorite(context) async {
    try {
      if ((!isFetched || isError) && !isLoading) {
        _isLoading = true;
        final res = await UserService().getFavoriteCandidat();
        if (res.statusCode == 401) return sessionExpired(context);
        if (res.statusCode != 200) throw "ERROR_SERVER";
        final jsonData = json.decode(res.body);
        _favorites = User.listFromJson(jsonData["favoris"]);
        _isError = false;
        _isFetched = true;
        _isLoading = false;
        notifyListeners();
      }
    } catch (e) {
      _isError = true;
      _isLoading = false;
      notifyListeners();
    }
  }

  void setFavorites(List<User> users) {
    _favorites = users;
    notifyListeners();
  }

  bool contains(User user) {
    for (int i = 0; i < _favorites.length; i++)
      if (_favorites[i].id == user.id) return true;
    return false;
  }

  void remove(User user) {
    _favorites.removeWhere((element) => element.id == user.id);
    notifyListeners();
  }

  void add(User user) {
    _favorites.removeWhere((element) => element.id == user.id);
    _favorites.add(user);
    notifyListeners();
  }

  void setIsError(bool value) {
    _isError = value;
    notifyListeners();
  }

  void setIsFetched(bool value) {
    _isFetched = value;
    notifyListeners();
  }
}
